...
def register_remaining_parsers(subparsers: argparse._SubParsersAction) -> None:
    ...
    """
    # NOTE: generators commands have been migrated to cli/generators.py
    ...
    tok_create = tok_sub.add_parser("create", help="Create tokenizer.")
    tok_create.add_argument("--output", required=True, help="Output path.")
    tok_create.add_argument("--split-addresses", action="store_true", help="Split addresses.")
    tok_create.add_argument("--max-length", type=int, default=2048, help="Max length.")
    tok_create.set_defaults(func=_tokenizer_create_command)

    tok_train = tok_sub.add_parser("train", help="Train tokenizer.")
    tok_train.add_argument("--input", required=True, help="Training corpus.")
    tok_train.add_argument("--output", required=True, help="Output path.")
    tok_train.add_argument("--tokenizer", help="Existing tokenizer.")
    tok_train.add_argument("--min-frequency", type=int, default=2, help="Min frequency.")
    tok_train.add_argument("--max-vocab-size", type=int, default=32000, help="Max vocab size.")
    tok_train.add_argument("--show-unknowns", action="store_true", help="Show unknown tokens.")
    tok_train.set_defaults(func=_tokenizer_train_command)

    tok_analyze = tok_sub.add_parser("analyze", help="Analyze text.")
    tok_analyze.add_argument("--tokenizer", required=True, help="Tokenizer path.")
    tok_analyze.add_argument("--text", help="Text to analyze.")
    tok_analyze.add_argument("--file", help="File to analyze.")
    tok_analyze.add_argument("--verbose", action="store_true", help="Verbose output.")
    tok_analyze.set_defaults(func=_tokenizer_analyze_command)

    tok_info = tok_sub.add_parser("info", help="Show tokenizer info.")
    tok_info.add_argument("--tokenizer", required=True, help="Tokenizer path.")
    tok_info.set_defaults(func=_tokenizer_info_command)

    # Encoder
    enc_parser = subparsers.add_parser("encoder", help="Encoder data processing.")
    enc_sub = enc_parser.add_subparsers(dest="encoder_command")

    enc_analyze = enc_sub.add_parser("analyze", help="Analyze data quality.")
    enc_analyze.add_argument("--input", required=True, help="Input JSONL.")
    enc_analyze.add_argument("--tokenizer", help="Tokenizer path.")
    enc_analyze.add_argument("--field", default="output", help="Field to analyze.")
    enc_analyze.add_argument("--detailed", action="store_true", help="Detailed stats.")
    enc_analyze.set_defaults(func=_encoder_analyze_command)

    enc_filter = enc_sub.add_parser("filter", help="Filter by quality.")
    enc_filter.add_argument("--input", required=True, help="Input JSONL.")
    enc_filter.add_argument("--output", required=True, help="Output JSONL.")
    enc_filter.add_argument("--min-tokens", type=int, help="Min tokens.")
    enc_filter.add_argument("--max-tokens", type=int, help="Max tokens.")
    enc_filter.add_argument("--max-unk-ratio", type=float, default=0.1, help="Max unknown ratio.")
    enc_filter.add_argument("--tokenizer", help="Tokenizer path.")
    enc_filter.set_defaults(func=_encoder_filter_command)

    enc_dedupe = enc_sub.add_parser("dedupe", help="Deduplicate samples.")
    enc_dedupe.add_argument("--input", required=True, help="Input JSONL.")
    enc_dedupe.add_argument("--output", required=True, help="Output JSONL.")
    enc_dedupe.add_argument("--field", default="output", help="Field to dedupe.")
    enc_dedupe.add_argument("--threshold", type=float, default=0.95, help="Similarity threshold.")
    enc_dedupe.set_defaults(func=_encoder_dedupe_command)

    enc_sample = enc_sub.add_parser("sample", help="Sample data.")
    enc_sample.add_argument("--input", required=True, help="Input JSONL.")
    enc_sample.add_argument("--output", required=True, help="Output JSONL.")
    enc_sample.add_argument("--n", type=int, required=True, help="Number of samples.")
    enc_sample.add_argument("--stratify-by", help="Field to stratify.")
    enc_sample.add_argument("--seed", type=int, default=42, help="Random seed.")
    enc_sample.set_defaults(func=_encoder_sample_command)

    enc_pipeline = enc_sub.add_parser("pipeline", help="Full processing pipeline.")
    enc_pipeline.add_argument("--input", required=True, help="Input JSONL.")
    enc_pipeline.add_argument("--output-dir", required=True, help="Output directory.")
    enc_pipeline.add_argument("--tokenizer", help="Tokenizer path.")
    enc_pipeline.add_argument("--min-tokens", type=int, default=20, help="Min tokens.")
    enc_pipeline.add_argument("--max-tokens", type=int, default=2048, help="Max tokens.")
    enc_pipeline.add_argument("--max-unk-ratio", type=float, default=0.1, help="Max unknown ratio.")
    enc_pipeline.add_argument("--dedupe-threshold", type=float, default=0.95, help="Dedupe threshold.")
    enc_pipeline.add_argument("--train-ratio", type=float, default=0.9, help="Train ratio.")
    enc_pipeline.add_argument("--seed", type=int, default=42, help="Random seed.")
    enc_pipeline.set_defaults(func=_encoder_pipeline_command)

    enc_train = enc_sub.add_parser("train", help="Train encoder.")
    enc_train.add_argument("--train", required=True, help="Training data.")
    enc_train.add_argument("--output", required=True, help="Output directory.")
    enc_train.add_argument("--tokenizer", required=True, help="Tokenizer path.")
    enc_train.add_argument("--val", help="Validation data.")
    enc_train.add_argument("--epochs", type=int, default=10, help="Epochs.")
    enc_train.add_argument("--batch-size", type=int, default=32, help="Batch size.")
    enc_train.add_argument("--learning-rate", type=float, default=1e-4, help="Learning rate.")
    enc_train.add_argument("--hidden-size", type=int, default=256, help="Hidden size.")
    enc_train.add_argument("--num-layers", type=int, default=4, help="Number of layers.")
    enc_train.add_argument("--num-heads", type=int, default=4, help="Number of heads.")
    enc_train.set_defaults(func=_encoder_train_command)

    # Entity
    entity_parser = subparsers.add_parser("entity", help="Entity extraction.")
    entity_sub = entity_parser.add_subparsers(dest="entity_command")

    ent_extract = entity_sub.add_parser("extract", help="Extract entities.")
    ent_extract.add_argument("--input", required=True, help="Input JSONL or ASM file.")
    ent_extract.add_argument("--output", help="Output JSONL.")
    ent_extract.add_argument("--include-hardware", action="store_true", help="Include hardware entities.")
    ent_extract.set_defaults(func=_entity_extract_command)

    ent_list = entity_sub.add_parser("list", help="List known entities.")
    ent_list.add_argument("--category", help="Filter by category.")
    ent_list.set_defaults(func=_entity_list_command)

    ent_search = entity_sub.add_parser("search", help="Search entities.")
    ent_search.add_argument("query", help="Search query (address or name).")
    ent_search.set_defaults(func=_entity_search_command)

    # Scoring
    scoring_parser = subparsers.add_parser("scoring", help="Quality scoring.")
    scoring_sub = scoring_parser.add_subparsers(dest="scoring_command")

    score_score = scoring_sub.add_parser("score", help="Score samples.")
    score_score.add_argument("--input", required=True, help="Input JSONL.")
    score_score.add_argument("--output", required=True, help="Output JSONL.")
    score_score.add_argument("--electra-model", help="ELECTRA model path.")
    score_score.add_argument("--base-rom", help="Base ROM for ASAR validation.")
    score_score.set_defaults(func=_scoring_score_command)

    score_analyze = scoring_sub.add_parser("analyze", help="Analyze scores.")
    score_analyze.add_argument("--input", required=True, help="Scored JSONL.")
    score_analyze.add_argument("--histogram", action="store_true", help="Show histogram.")
    score_analyze.set_defaults(func=_scoring_analyze_command)

    # Pipeline
    pipeline_parser = subparsers.add_parser("pipeline", help="Data pipeline.")
    pipeline_sub = pipeline_parser.add_subparsers(dest="pipeline_command")

    pipe_run = pipeline_sub.add_parser("run", help="Run pipeline.")
    pipe_run.add_argument("--config", required=True, help="Pipeline config TOML.")
    pipe_run.add_argument("--dry-run", action="store_true", help="Show what would be done.")
    pipe_run.set_defaults(func=_pipeline_run_command)

    pipe_status = pipeline_sub.add_parser("status", help="Show pipeline status.")
    pipe_status.add_argument("--dir", required=True, help="Pipeline output directory.")
    pipe_status.set_defaults(func=_pipeline_status_command)

    # Evaluation
    eval_parser = subparsers.add_parser("evaluation", help="Model evaluation.")
    eval_sub = eval_parser.add_subparsers(dest="evaluation_command")

    eval_run = eval_sub.add_parser("run", help="Run evaluation.")
    eval_run.add_argument("--input", required=True, help="Input JSONL.")
    eval_run.add_argument("--output", required=True, help="Output JSON.")
    eval_run.add_argument("--electra-model", help="ELECTRA model.")
    eval_run.add_argument("--base-rom", help="Base ROM.")
    eval_run.set_defaults(func=_evaluation_run_command)

    eval_compare = eval_sub.add_parser("compare", help="Compare evaluations.")
    eval_compare.add_argument("--baseline", required=True, help="Baseline JSONL.")
    eval_compare.add_argument("--candidate", required=True, help="Candidate JSONL.")
    eval_compare.add_argument("--output", help="Output JSON.")
    eval_compare.add_argument("--electra-model", help="ELECTRA model.")
    eval_compare.set_defaults(func=_evaluation_compare_command)

    eval_human_create = eval_sub.add_parser("human-create", help="Create human eval batch.")
    eval_human_create.add_argument("--input", required=True, help="Input JSONL.")
    eval_human_create.add_argument("--output", required=True, help="Output CSV.")
    eval_human_create.add_argument("--n", type=int, default=50, help="Number of samples.")
    eval_human_create.add_argument("--strategy", default="uncertainty", choices=["uncertainty", "random", "stratified"], help="Sampling strategy.")
    eval_human_create.add_argument("--electra-model", help="ELECTRA model for uncertainty.")
    eval_human_create.set_defaults(func=_evaluation_human_create_command)

    eval_human_import = eval_sub.add_parser("human-import", help="Import human ratings.")
    eval_human_import.add_argument("--input", required=True, help="Rated CSV.")
    eval_human_import.add_argument("--original", required=True, help="Original JSONL.")
    eval_human_import.add_argument("--output", required=True, help="Output JSONL.")
    eval_human_import.add_argument("--min-rating", type=float, default=0.6, help="Min rating to include.")
    eval_human_import.set_defaults(func=_evaluation_human_import_command)

    # Active Learning
    al_parser = subparsers.add_parser("active-learning", help="Active learning utilities.")
    al_sub = al_parser.add_subparsers(dest="active_learning_command")

    al_sample = al_sub.add_parser("sample", help="Sample by uncertainty.")
    al_sample.add_argument("--input", required=True, help="Input JSONL.")
    al_sample.add_argument("--output", required=True, help="Output JSONL.")
    al_sample.add_argument("--n", type=int, default=100, help="Number of samples.")
    al_sample.add_argument("--strategy", default="uncertainty", choices=["uncertainty", "random", "diverse"], help="Strategy.")
    al_sample.add_argument("--electra-model", help="ELECTRA model.")
    al_sample.set_defaults(func=_active_learning_sample_command)

    al_curriculum = al_sub.add_parser("curriculum", help="Curriculum learning split.")
    al_curriculum.add_argument("--input", required=True, help="Input JSONL.")
    al_curriculum.add_argument("--output-dir", required=True, help="Output directory.")
    al_curriculum.add_argument("--stage", choices=["simple", "moderate", "complex", "advanced"], help="Filter to stage.")
    al_curriculum.set_defaults(func=_active_learning_curriculum_command)

    al_queue_add = al_sub.add_parser("queue-add", help="Add to annotation queue.")
    al_queue_add.add_argument("--input", required=True, help="Input JSONL.")
    al_queue_add.add_argument("--queue", required=True, help="Queue file.")
    al_queue_add.add_argument("--electra-model", help="ELECTRA model.")
    al_queue_add.set_defaults(func=_active_learning_queue_add_command)

    al_queue_get = al_sub.add_parser("queue-get", help="Get batch from queue.")
    al_queue_get.add_argument("--queue", required=True, help="Queue file.")
    al_queue_get.add_argument("--output", required=True, help="Output JSONL.")
    al_queue_get.add_argument("--n", type=int, default=10, help="Batch size.")
    al_queue_get.set_defaults(func=_active_learning_queue_get_command)

    al_queue_status = al_sub.add_parser("queue-status", help="Show queue status.")
    al_queue_status.add_argument("--queue", required=True, help="Queue file.")
    al_queue_status.set_defaults(func=_active_learning_queue_status_command)

    # Generator (model-based)
    gen_parser = subparsers.add_parser("generator", help="Model-based generation.")
    gen_sub = gen_parser.add_subparsers(dest="generator_command")

    gen_model = gen_sub.add_parser("model", help="Generate with trained model.")
    gen_model.add_argument("--instruction", "-i", required=True, help="Instruction.")
    gen_model.add_argument("--model", "-m", help="Model path.")
    gen_model.add_argument("--type", "-t", choices=["mlx", "huggingface", "llama_cpp", "api"], default="api", help="Model type.")
    gen_model.add_argument("--provider", "-p", choices=["gemini", "claude", "openai"], default="gemini", help="API provider.")
    gen_model.add_argument("--model-name", help="Model name.")
    gen_model.add_argument("--api-key", help="API key.")
    gen_model.add_argument("--context", "-c", help="Context.")
    gen_model.add_argument("--temperature", type=float, default=0.7, help="Temperature.")
    gen_model.add_argument("--max-tokens", type=int, default=1024, help="Max tokens.")
    gen_model.add_argument("--output", "-o", help="Output file.")
    gen_model.set_defaults(func=_generator_model_command)

    gen_knowledge = gen_sub.add_parser("knowledge", help="Generate with knowledge context.")
    gen_knowledge.add_argument("--instruction", "-i", required=True, help="Instruction.")
    gen_knowledge.add_argument("--entities", "-e", help="Required entities (comma-separated).")
    gen_knowledge.add_argument("--suggest", action="store_true", help="Only suggest entities.")
    gen_knowledge.add_argument("--provider", "-p", choices=["gemini", "claude", "openai"], default="gemini", help="API provider.")
    gen_knowledge.add_argument("--api-key", help="API key.")
    gen_knowledge.add_argument("--temperature", type=float, default=0.7, help="Temperature.")
    gen_knowledge.add_argument("--max-tokens", type=int, default=1024, help="Max tokens.")
    gen_knowledge.add_argument("--output", "-o", help="Output file.")
    gen_knowledge.set_defaults(func=_generator_knowledge_command)

    gen_batch = gen_sub.add_parser("batch", help="Batch generation.")
    gen_batch.add_argument("--input", required=True, help="Input file with instructions.")
    gen_batch.add_argument("--output", "-o", required=True, help="Output JSONL.")
    gen_batch.add_argument("--provider", "-p", choices=["gemini", "claude", "openai"], default="gemini", help="API provider.")
    gen_batch.add_argument("--api-key", help="API key.")
    gen_batch.add_argument("--temperature", type=float, default=0.7, help="Temperature.")
    gen_batch.add_argument("--max-tokens", type=int, default=1024, help="Max tokens.")
    gen_batch.set_defaults(func=_generator_batch_command)
